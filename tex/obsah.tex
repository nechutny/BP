\chapter{Úvod}
	...
	Cílem této práce je øevod knihoven v~PHP na ekvivalent v~jazyce C++ pøipravený pro zkompilování jako roz¹íøení PHP. Zadání vychází ze skuteènosti, ¾e èastým zpùsobem vývoje aplikace je pou¾ití jedné konkrétní verze knihovny, v~které není tøeba provádìt úpravy. V~dùsledku pak výhody plynoucí z~interpretace nejsou tøeba, a naopak nevýhody znamenají propad výkonu.
...
\chapter{Konkurenèní øe¹ení}
	...
	\section{phc}
		Phc je kompilátor podmno¾iny jazyka PHP do spustitelných binárních souborù. Odstraòuje závislost na interpretru PHP a tak umo¾òuje pohodlnou distribuci CLI skriptù. Vývoj tohoto nástroje zapoèali v~roce 2005 vývojáøi Edsko de Vries a John Gilbert. Postupnì se do vývoje zapojilo na 21 vývojáøù, poslední verze byla vydána v~roce 2011\cite{PhcContributing}. Od té doby se pouze mno¾í chybové reporty a projekt není udr¾ován\cite{PhcIssues}.
		...
		\subsection{Dotazy}
		Pøi analýze tohoto øe¹ení jsem se rozhodl kontaktovat vývojáøe a provést s~nimi krátký rozhovor, v~kterém bych rád zjistil odpovìdi na nìkolik otázek. Zamý¹lel jsem zjistit v~jakém stavu se tento projekt nachází, jaké byly plány do budoucna a proè do¹lo k~jeho pozastavení. Dále mne zajímalo, zda i dnes pova¾uje zainteresovaná osoba tento koncept za prakticky pou¾itelný. Od øíjna roku 2015 jsem se pokou¹el kontaktoval postupnì 5 bývalývh vývojáøù tohoto projektu, ale bohu¾el se mi nepodaøilo získat odpovìdi. A¾ v kvìtnu 2016 se mi podaøilo navázat prostøednictvím Twitteru kontakt s Edsko de Vries, který pøislíbil odpovìdìt na dotazy. Odpovìd jsem bohu¾el obdr¾el pouhých nìkolik dní, pøed dokonèením této práce a tak jsem nemohl dùkladnì prozkoumat.

		V odpovìdi mi zaslal mimo jiné odkaz na dizertaèní práci dal¹ího vývojáøe phc, která obsahuje velké mno¾ství informací vhodných pro návrh pøekladaèe, který je cílem této práce. Text e-mailu v originálním znìní pøikládám.
		...
		\begin{quotation}\small{}
			...
			Hi Stanislav,
			\\ \\
			I started work on phc a long time ago, together with a friend of mine, John Gilbert. We were both PhD students at the time working on compiler related topics. Later Paul Biggar joined the team and he made phc into the main topic of his PhD research, so his thesis contains a lot of detail:
			\\ \\
			\url{https://paulbiggar.com/research/thesis.pdf}
			\\ \\
			You'll find other resources on his website as well (\url{https://paulbiggar.com/research}) but I suspect most of that information is also in the thesis.
			\\ \\
			To answer your specific questions:
			\\ \\
			> 1) What was the planes for future?
			\\ \\
			We didn't really have much of a plan.. It seemed like an interesting research topic and so we started work on it :)
			\\ \\
			> 2) Why is not project in active development?
			\\ \\
			I guess all of us got busy with other things. Also, the premise of phc was to compile all of PHP, accurately, and that turns out to be a very difficult task indeed (Paul's thesis goes into detail on these problems).
			\\ \\
			...
			\\ \\
			I think you'll find answers to both of these questions in Paul's thesis. My personal take on this right now is that if you want to compile PHP, you have a choice between either limiting the subset of PHP that you support (this is what Hiphop did -- \url{https://en.wikipedia.org/wiki/HipHop_for_PHP}), or else accept that you're not going to get much better performance than interpreting because the PHP semantics is so convoluted that it's very difficult to do any kind of static analysis. (Perhaps this is also the reason why Facebook moved to a JIT compiler instead, \url{https://en.wikipedia.org/wiki/HipHop_Virtual_Machine} -- of course a JIT compiler has more knowledge about the dynamic state of the program than a ahead--of--time compiler does.)
			\\ \\
			Hope that helps,
			\\ \\
			Edsko
...
\chapter{Rozdíly PHP a C++}
	I~pøes velkou podobnost je rozdílù mezi obìma jazyky celá øada a je tøeba se s~nimi vypoøádat.
	...
	\section{Promìnné}
		...
		Z~tìchto specifikací názvù promìnných vyplývá nìkolik omezení, které je tøeba vyøe¹it. Pou¾ití znakù z~rozsahu s~kódem \function{0x7f -- 0xff} není v~kódu PHP pøíli¹ èasté, a mù¾eme tedy pro úèely nástroje zobrazit u¾ivateli upozornìní, ¾e tento název není podporován. Název promìnné PHP, který je v~jazyce C++ klíèovým slovem, èi zaèíná èíslicí, je mo¾no upravit prefixem. Tím dojde i k~zpøehlednìní kódu, kdy bude jasnì rozli¹itelné, které promìnné jsou ekvivalentem promìnných z~PHP a které byly pøidány v~rámci transformace. Pro tyto úèely byl zvolen prefix \function{phpVar\_}.

		Dal¹í èástí, kterou je tøeba vyøe¹it, je rozdílný obor platnosti promìnných. Specifikace jazyka C++ definuje tzv. Lokální obor, který má rozsah bloku uvnitø \function{\{} a \function{\}}. Tedy promìnná definovaná v~tìle funkce je dostupná uvnitø dal¹ích vnoøených blokù a¾ do konce tìla funkce. V~pøípadì tìla cyklu nebo podmínky je pak promìnná definovaná v~tomto tìle dostupná do jeho konce. Dále je v~C++ rozsah výrazu (Statement scope), kdy promìnná definovaná uvnitø \function{(} a \function{)} u~konstrukcí \function{for}, \function{while}, \function{if} a \function{switch} je platná do konce tìla\cite{CppBook}. Jazyk PHP obsahuje lokální obor funkce, kdy v¹echny promìnné, které jsou definovány uvnitø bloku, nebo by v~C++ mìly platnost pouze v~rozsahu výrazu, jsou od daného místa dostupny dále i v~nadøazených blocích\cite{phpVariables}.
...
\chapter{Integrace do PHP}
	...
	Interpret PHP umo¾òuje zavádìt dynamické knihovny tzv. moduly, které roz¹iøují dostupné funkce a tøídy v~interpretovaném prostøedí. Naètení je mo¾né prostøednictvím konfiguraèní direktivy \function{ex\-ten-\-sion=nazev.so} v~konfiguraèním souboru \function{php.ini}, èi v~dobì bìhu funkcí \function{dl("na\-zev.so")}. Roz¹íøení obsahují tzv. entry point, tedy adresu, kam v~momentì po¾adavku na naètení modulu skoèí tok interpretru a oèekává se vrácení datové struktury, která obsahuje informace o~dostupných funkcích a tøídách. Zend Engine\footnote{\url{http://www.zend.com/en/community/php}}, jádro PHP, je napsáno primárnì v~jazyce C, tak¾e je tøeba dodr¾et kompatibilní zpùsob volání a navrácení výsledku. V~pøípadì této práce bude z~dùvodu jazyka C++ nutno pou¾ít konstrukci \function{extern "C"}\cite{PhpBookRozsireni}\cite{PhpExtensionStructure} \cite{phpExtensionLife}.
...
$a = TRUE AND FALSE; // ($a = TRUE) AND FALSE   -> $a == TRUE
		...
		\subsection{Matematické operátory}
			Matematické operátory jako \function{+}, \function{-}, \function{*} a dal¹í jsou øe¹eny bez nutnosti výrazné úpravy kódu. Pouze je doplnìna pøípadná konverze datových typù na základì výsledkù analýzy. Výjimkou je dìlení a mocnina, které jsou popsány v~následujících oddílech.
		...
		\subsection{Porovnání}
			...
			PHP pro porovnání zval struktur s~konverzí pou¾ívá funkci \function{compare\_function(result, a, b TSRMLS\_CC);}. Ta napøed provede sjednocení datových typù hodnot. Pokud se jedná o~øetìzec, tak je pou¾ita funkce \function{zend\_is\_numeric}, která detekuje v~øetìzci pøípadné celé èíslo, nebo desetinné a vrátí výsledek. Tato funkce je navr¾ena jako koneèný automat pøecházející mezi stavy. Následnì jsou v~závislosti na typu provedeny konverze na spoleèný datový typ a provedeno porovnání. Informace o~fungování a návrhu porovnávání byla zji¹tìna analýzou zdrojových kódù interpretru PHP.
...
\chapter{Implementace}
	Pro implementaci byl zvolen jazyk PHP, v~kterém je mo¾né provádìt rychle implementaci, a poskytuje potøebné funkce. Velkou výhodu pro implementaci je \function{token\_get\_all()}, která umo¾òuje naparsovat zdrojový kód a pøevést jej na tokeny. Po získání tokenù je provedena analýza rekurzivním sestupem shora dolu. Pøi analýze je generován abstraktní syntaktický strom pro generátor kódu. Parser vyu¾ívá volání precedenèní analýzy, která provádí kontrolu výrazù a provádí také tvorbu derivaèního stromu, kterého je následnì vyu¾ito pro tvorbu abstraktního syntaktického stromu výrazù. Po získání v¹ech potøebných informací ze zdrojového kódu je pøistoupeno k~analýze výrazù pro detekci datových typù a následnì provedeno generování C++ kódu.

	Prvnì jsem provedl analýzu nìkolika hotových øe¹ení parsování zdrojových kódù, ale zjistil jsem, ¾e rychlej¹ím a pohodlnìj¹ím pro dal¹í vývoj bude tvorba vlastního øe¹ení. Prozkoumaná øe¹ení byla èasto velmi obecná pro pokrytí co nejvìt¹í èásti jazyka PHP a nedetekovala pak nìkteré chybné konstrukce. Dále pak jejich výstup neobsahoval dostatek potøebných informací pro urèování datových typù a vhodné sestavování C++ kódu. Mezi zkoumanými øe¹eními bylo i PHP-Parser\footnote{\url{https://github.com/nikic/PHP-Parser}}, jeho¾ autorem je Nikita Popov, který také pøispívá do PHP interpretru a je jedním z~autorù knihy \uv{PHP Internals Book}, zabývající se tvorbou PHP roz¹íøení. Zajímavým a vhodným øe¹ením s~dobrým výstupem se ukázal PHP Analyzer\footnote{\url{https://scrutinizer-ci.com/docs/tools/php/php-analyzer/}} od spoleènosti Scrutinizer, který provádí velmi podrobnou analýzu. Problémem v¹ak byla vysoká cena, komplikující zámìr poskytnout výsledné øe¹ení jako open source produkt dostupný zdarma.
	...
	\section{Analýza zdrojových kódù PHP}
		...
		Vlastní parser vyu¾ívá metody rekurzivního sestupu shora dolù. Pøi prùchodu sestavuje abstraktní syntaktický strom. Kdy¾ narazí na výraz, tak je pou¾ita zásobníková precedenèní analýza. Její derivaèní strom, který je výstupem, je pak opìt pøeveden na abstraktní syntaktický strom a zaèlenìn do stromu vzniklého v~parseru.

		Pro ulo¾ení informací o~promìnných - datové typy, pou¾ití apod. jsou pou¾ity instance tøídy \function{Variable} ulo¾ené v~kontejneru, který tvoøí instance tøídy \function{Scope}. Informace jsou získávány z~výstupu precedenèní analýzy, jeliko¾ v~PHP mù¾e být pou¾ita promìnná ve výrazu i bez definice. Pak dojde, v~závislosti na konfiguraci, k~vypsání varování a hodnota takové promìnné je rovna \function{NULL}.
	...
	\section{Generování kódu}
		...
		\function{FileGenerator} je pou¾it pro generování výstupního C++ souboru. Na zaèátek souboru doplòuje potøebné konstrukce pro vlo¾ení hlavièkových souborù a dal¹ích C++ funkcí a tøíd vytvoøených pro potøeby pøekladu. Dále volá generování kódu funkcí a následnì ve funkci \function{get\_module} exponuje jednotlivé pøelo¾ené funkce interpretovanému prostøedí.
...
\end{lstlisting}
		...
		\function{ExprGenerator} generuje výraz na základì výsledku z~precedenèní analýzy a sady pravidel zmínìných v~kapitole \ref{phpTransformToCpp}. Rekurzivnì projde strom výrazu a vrátí kód.
	...
	\section{Testování}
		...
		Testy jsou umístìny u~zdrojových kódù v~adresáøi \uv{tests} a rozèlenìny podle specifické èásti, kterou testují.  Testy pokrývají základní konstrukce -- výstup, komentáøe, konstanty, funkce, vkládání, výrazy, argumenty. Dále jsou testy zamìøené na pole, cykl for, foreach, while, funkce, podmínky, precedenèní analýzu nebo práci s~øetìzci. Mezi dal¹í testy testy v~jazyce C++ pro ovìøení funkènosti èásti implementované v~C++ a testy algoritmù -- soubory s~algoritmem implementovaným v~PHP pro testování výkonu a komplexnìj¹ího bloku kódu.



	\section{Experimentování}
		Po dokonèení zamý¹leného rozsahu jsem se zamìøil na testování pøekladu na pøipravených algoritmech i èástech pou¾ívaných knihoven, nebo èástech samotného øe¹ení. Najít v¹ak knihovnu, èi aplikací, její¾ èást by bylo mo¾né pøelo¾it, se ukázalo jako velmi obtí¾né. Pøíèinou byl objektový návrh vìt¹iny knihoven, èi èasté pou¾ívání referencí, anonymních funkcí a globálních promìnných, které nejsou v~souèasné implementaci pøekladaèe podporovány.

		Ve¹keré experimenty byly provádìny na poèítaèi vybaveným procesorem Intel(R) Core(TM) i7-4702MQ CPU @ 2.20GHz, 16 GB RAM 1 600 MHz, SSD s~zápisem 480 MB/s a ètením 540 MB/s. Operaèním systémem je Fedora 21 s~jádrem verze 4.1.13-100.fc21.x86\_64. Verze PHP interpretru je 5.6.15 s~Zend Engine v2.6.0 a Xdebug v2.3.3. Jako kompilátor pøelo¾ených C++ zdrojových kódù bylo pou¾ito GCC 4.9.2 20150212 (Red Hat 4.9.2-6). Pro kompiler byly pou¾ity pøepínaèe \function{-Wall -c -O2 -std=c++11 -fpic -o}.

		Bohu¾el se ukázalo, ¾e podmno¾ina jazyka PHP nebyla vybrána zcela optimálnì a absence konstrukce \function{isset} a \function{unset} znemo¾nila pøeklad vìt¹iny knihoven. Pro experimenty a testování jsem proto pou¾il nìkteré algoritmy a vzorce, které zastupují knihovní funkce.
				\\ \\
				\textbf{Faktoriál}

					Pro výpoèet faktoriálu byla pou¾ita implementace s~cyklem typu for provádìjící násobení s~ulo¾ením mezivýsledku do promìnné. Pro výpoèet byl zvolen faktoriál èísla 5 000 000. Kód pro mìøení byl pou¾it \ref{code:factorialBenchmark}.

					Èas prvních 3 testù zkompilované verze je: 50.831 s, 51.555 s, 50.380 s. Interpretovaná verze: 52.301 s, 49.888 s, 50.123 s. Po úpravì datových typù (zmìna \function{Php::Value} za \function{double}) byly výsledky testu 2.149 s, 2.144 s, 2.139 s. Výsledek tohoto testu ukázal zpomalení 0.3\% zkompilované verze. Zajímavìj¹í je ov¹em výsledek verze upravené o~správné datové typy, který potøeboval pro svùj bìh pouze 4.2\% èasu interpretované verze. Tato verze u¹etøí 95.7\% pùvodnì potøebného èasu a bylo tøeba provést jednoduchou zmìnu na 2 øádcích vygenerovaného kódu.

\begin{lstlisting}[caption=Test pro mìøení rychlosti výpoètu faktoriálu, label=code:factorialBenchmark, language=PHP]
...
\end{lstlisting}
				...
				\textbf{Fibonacciho èíslo}

					Algoritmus pro výpoèet n-tého èísla Fibonacciho øady byl zvolen zámìrnì neefektivní formou rekurzivního algoritmu vycházejícícho z~definice \ref{fibonacci}
  ...
  \right.
					...
					Pro test byl zvolen výpoèet 27. prvku øady. Provedeno 100 výpoètù a zmìøen èas kódem \ref{code:fibonacciBenchmark}. Èas zkompilované verze: 84.709 s, 84.177 s, 84.243 s. Intepretovaná verze: 54.155 s, 53.285 s, 53.332 s. V~tomto pøípadì zafungovala detekce datových typù správnì a nebyla tedy vytvoøena verze obsahující ruènì zoptimalizované datové typy. V~pøípadì takto èasté rekurze by ov¹em mohlo být vhodné zoptimalizovat volání funkce -- nevolat funkci prostøednictvím konstrukce \function{Php::Call}, ale volat pøímo její C++ implementaci. Proto vznikla optimalizovaná verze tímto postupem a byla zmìøena. Èasy této implementace jsou: 84.702 s, 83.288 s, 83.765 s.

					Pøelo¾ená verze dosáhla zpomalení o~57.4\% oproti interpretované verzi. Ukázalo se, ¾e pøelo¾ené funkce s~velkým mno¾stvím rekurze jsou pomalej¹í, ne¾ interpretované. Do optimalizace, kdy bude z~volání funkce vynechána knihovna PHP-CPP, resp. Zend engine, jsem vkládal nadìje na zrychlení. Ukázalo se, ¾e tato implementace je stále výraznì pomalej¹í, ne¾ interpretovaná i kdy¾ bylo dosa¾eno zrychlení o~0.6\% oproti pouze pøelo¾ené verzi.

					\begin{lstlisting}[caption=Test pro mìøení rychlosti výpoètu fibonacciho èísla, label=code:fibonacciBenchmark, language=PHP]
$start = microtime(TRUE);
for($i = 0; $i < 100; $i++)
{
	fibonacci(27);
...
\end{lstlisting}
				...
				\textbf{Arcus sinus}
					...
					Algoritmus pro výpoèet byl pou¾it taylorùv polynom s~vzorcem \ref{asinPolynom}, který zejména pro hodnoty limitnì se blí¾ící 1 má pomalou aproximaci výsledku. Pro pøesnost byla pou¾ita konstanta $1^-15$.

					\begin{equation} \label{asinPolynom}
							arcsin(x) = x + \frac{1}{2} * \frac{x^{3}}{3} + \frac{1*3}{2*4} * \frac{x^{5}}{5} + \frac{1 * 3 * 5}{2 * 4 * 6} * \frac{x^{7}}{7} + ... =  \sum\limits_{n=0}^{\inf} (\frac{(2n)!}{2^{2n}(n!)^2} * \frac{x^{2n+1}}{2n+1}), |z| < 1
					\end{equation}

					Pro mìøení byl pou¾it kód \ref{code:asinBenchmark}, který provedl 100 výpoètù hodnoty arcus sinus pro èíslo 0,99999. Potøebný èas pro tìchto 100 výpoètù zmìøí s~dostateènou pøesností. Výsledky pro 3 mìøení zkompilované verze jsou: 133.469 s, 128.166 s, 133.269 s~a pro interpretovanou verzi 126.870 s, 126.772 s, 126.957 s. Po úpravì vygenerovaného zdrojového kódu, která spoèívala v~zmìnì datových typù (zmìna \function{Php::Value} za \function{double}) byly výsledky testu 113.505 s, 114.399 s, 114.408 s.

					V~pøípadì zlep¹ení detekce datových typù je tedy mo¾nost u~tohoto algoritmu dosáhnout zkrácení výpoèetního èasu na 89.94 \% pùvodního èasu. Souèasná implementace pøekladaèe se ukázala jako nejpomalej¹í a naopak pøidává zpomalení ve vý¹i 3.75 \%.

					\begin{lstlisting}[caption=Test pro mìøení rychlosti výpoètu Arcus sinus, label=code:asinBenchmark, language=PHP]
$start = microtime(TRUE);
for($i = 0; $i < 100; $i++)
{
	my_asin(0.99999);
...
\end{lstlisting}
	...
	\section{Návrhy na zlep¹ení}

		Ukázalo se, ¾e pøedpokládané úzké hrdlo ve formì tøídy \function{Php::Value} vyu¾ívající internì \function{zval} struktury a funkce interpretru je reálným problémem. Proto jsem zapoèal implementaci vlastní tøídy s~pøetí¾enými operátory vyu¾ívající cachování výsledkù pøi konverzi typù a detekci celých a desetinných èísel v~øetìzcích. Èásteènì implementovaná tøída je ji¾ dostupná v~zdrojových kódech projektu v~souboru \function{cpp/PhpValue.cpp}. Bohu¾el kód není natolik kompletní, aby bylo mo¾né zmìøit dopad na rychlost.

		Dal¹ím problémem byla nepøesná detekce datových typù. V~pøípadì zpøesnìní výsledkù by do¹lo k~generování optimálnì¹ího kódu a zlep¹ení výsledkù, co¾ ukázal experiment. Pro zlep¹ení detekce datových typù se nabízí nìkolik nìkolik øe¹ení. Prvním je roz¹íøení pravidel o~dal¹í pravidla, pro zpøesnìní detekce. Tento postup bude vy¾adovat dal¹í podrobné zkoumání zdrojových kódù PHP, pro zji¹tìní pøesného chování operátorù i v~okrajových situacích. Dal¹í mo¾ností je zavedení fuzzy logiky do detekce datových typù a následné generování nìkolika variant kódu -- pro pravdìpodobnìj¹í kombinaci datových typù by byl vygenerován optimalizovaný kód s~datovými typy a pro ménì pravdìpodobný pak obecý kód s~pou¾itím \function{Php::Value}. Obdobné øe¹ení jsem z~poèátku zavrhl pro vysoký poèet mo¾ných kombinací, ale v~pøípadì generování jen pro více pravdìpodobné kombinace by tato optimalizace mohla mít smysl.

		Zajímavým zpùsobem, jak detekovat datové typy v~kódu by mohlo být vytvoøení vlastního analytického roz¹íøení PHP, které by sledovalo vykonávání interpretované aplikace. Tento nástroj by se zamìøil na hodnoty ulo¾ené v~zval strukturách interpretru PHP pøi bì¾ném provozu PHP aplikace. Po nasbírání dostateènì relevantního vzorku by byla provedena analýza typù a výsledek vyu¾it pro generování kódu. S~pøihldénutím k~pravdìpodobnosti, ¾e ojde k~zavolání nìkteré funkce s~argumentem jiného datového typu, ne¾ jaký byl v~analyzovaných datech by byla opìt generována i verze vyu¾ívající \function{Php::Value}.

		Navr¾ené, a v~sekci experimentování otestované, øe¹ení zamý¹lející vynechat PHP-CPP z~procesu volání funkce, v~pøípadì volání pøelo¾ené funkce z~pøelo¾eného kódu, se ukázalo jako neopodstatnìné. Provedené mìøení ukázalo zrychlení o~pouhých 0.6\% pùvodního èasu.
...
\chapter{Závìr}
	Výsledkem této práce je pøekladaè, který je schopen pøelo¾it podmno¾inu jazyka PHP do C++. Nìkteré pøelo¾ené konstrukce jsou rychlej¹í, ale bohu¾el práce s~poli a volání funkcí je stále pomalej¹í. Pøi práci na projektu byla vytvoøena sada pravidel pro detekci datových typù na základì výrazù a navr¾eno nìkolik dal¹ích zpùsobù detekce datových typù pùvodnì slabì typovaných promìnných. Dal¹ím výsledkem práce je navr¾ení a odzkou¹ení pravidel pro transformaci kódu øe¹ící rozdíly obou jazykù.

	Praktické pou¾ití projektu v~souèasném rozsahu je komplikované, jeliko¾ výsledný kód není v¾dy rychlej¹í a není podporována dostateènì velká mno¾ina jazyka, která je zastoupena v~reálných aplikacích. Ukázalo se, ¾e i kdy¾ se pøi výbìru podmno¾iny zdál výbìr dostateènì ¹iroký, tak pro reálné aplikace je znaènì nedostaèující.

	Souèasný stav ov¹em plánuji zmìnit, jeliko¾ nástroj vzniklý v~rámci této práce hodlám zveøejnit zdarma jako open source projekt pod licencí Apache 2.0 s~placenou technickou podporou. V~dobì dokonèování této práce je ji¾ na adrese \url{www.php2cpp.net} rozpracovaný web pro prezentaci øe¹ení. Zájem o~tento nástroj projevilo hned nìkolik zástupcù firem na studentské konferenci Excel@FIT, na které jsem demonstroval své øe¹ení. Výsledek má tedy, v~pøípadì pokraèujícího vývoje, komerèní potenciál. V~blízké dobì mám v~úmyslu zlep¹it detekci datových typù za pou¾ití postupù navr¾ených v~této práci a dokonèit novou implementaci tøídy pro dynamické datové typy. Zamý¹lím také prezentovat toto øe¹ení na dal¹ích konferencích zamìøených na PHP vývojáøe. Jednou z~takových akcí, na ní¾ je úèast ji¾ v~jednání, je PoSobota, na které se neformálnì scházejí vývojáøi, nejen pou¾ívající Nette Framework, a pøedná¹ejí o~zajímavých technických øe¹eních.
