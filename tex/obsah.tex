...
\chapter{Vybrané konstrukce PHP}
	Vkládání prostøednictvím konstrukcí \function{include}, \function{require} a jejich variant pro unikátní vlo¾ení bude z dùvodu komplexity implementováno jen v omezené míøe. Prvním logickým omezením vycházejícím z návrhu tohoto nástroje je schopnost provést vlo¾ení pouze souborù, které jsou dostupné v momentì kompilace a jejich¾ cesta je známá. Nebude tedy mo¾ná transformace funkce \function{includeNe} uvedené v kódu \ref{code:phpIncludeFunction}, ale funkce \function{includeAno} v pøípadì existence souborù \function{prvni.php} a \function{druhy.php} ano.

	Druhým omezením vkládání souborù je pouze èásteèná implementace varianty \function{include\_once} a \function{require\_once}, kdy nebude mo¾né zcela zajistit neopakované vlo¾ení v dobì bìhu aplikace. V dobì transformace PHP do C++ bude toto chování kontrolováno a dojde pouze k prvnímu vlo¾ení. V dobì vykonávání zkompilované funkce v prostøedí interpretru PHP v¹ak nebude mo¾né zamezit vlo¾ení, jeliko¾ systém roz¹íøení PHP neumo¾nuje ovlivnovat tyto èásti.
...
\end{lstlisting}
	...
	\section{Tøídy}
		...
		TODO: Bude podpora vytváøení instancí PHP tøíd v C++, èi jen pùjde o definici tøíd bez mo¾nosti je v transofmrovaném kódu pou¾ít? Nároènost vs. pou¾itelnost.

		\begin{itemize}
			\item T\_CLASS				\function{class}
			\item T\_EXTENDS			\function{extends}
			\item T\_IMPLEMENTS			\function{implements}

			\item T\_CONST				\function{const}

			\item T\_INTERFACE			\function{interface}

			\item T\_NAMESPACE			\function{namespace}
			\item T\_NS\_SEPARATOR		\function{$\backslash$}

			\item T\_USE				\function{use}

			%\item T\_NEW				\function{new}

			\item T\_PRIVATE			\function{private}
			\item T\_PUBLIC				\function{public}
			\item T\_PROTECTED			\function{protected}

			\item T\_STATIC				\function{static}

			\item T\_DOUBLE\_COLON		\function{::}
			\item T\_OBJECT\_OPERATOR	\function{-\textgreater}
